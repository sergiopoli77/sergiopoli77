<div align="center">

<!-- ANIMATED HEADER BANNER -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0D1117,50:1a1a2e,100:16213e&height=200&section=header&text=Sergio%20Poli&fontSize=70&fontColor=61DAFB&fontAlignY=40&desc=LLM%20Developer%20%7C%20AI%20Engineer%20%7C%20Builder&descAlignY=65&descSize=18&descColor=8b949e&animation=fadeIn" />

<!-- TYPING ANIMATION -->
<a href="https://github.com/sergiopoli77">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&pause=1000&color=61DAFB&center=true&vCenter=true&width=600&lines=Building+Intelligent+AI+Systems+%F0%9F%A4%96;LLM+%2B+RAG+%2B+LangChain+%2B+WhatsApp+API;Turning+Ideas+into+Real+AI+Products+%F0%9F%9A%80;Open+to+Collaborate+%E2%80%94+Let%27s+Build!+%F0%9F%94%A5" alt="Typing SVG" />
</a>

<br/>

<!-- SOCIAL BADGES -->
<p>
  <a href="https://discord.com/users/581871647066423297">
    <img src="https://img.shields.io/badge/Discord-581871647066423297-7289DA?style=for-the-badge&logo=discord&logoColor=white&labelColor=0D1117"/>
  </a>
  &nbsp;
  <a href="https://instagram.com/sergiooyp">
    <img src="https://img.shields.io/badge/Instagram-@sergiooyp-E4405F?style=for-the-badge&logo=instagram&logoColor=white&labelColor=0D1117"/>
  </a>
  &nbsp;
  <img src="https://img.shields.io/badge/Status-Open%20to%20Work-00D26A?style=for-the-badge&labelColor=0D1117"/>
</p>

<!-- PROFILE VIEWS COUNTER -->
<img src="https://komarev.com/ghpvc/?username=sergiopoli77&style=for-the-badge&color=61DAFB&label=PROFILE+VIEWS&labelColor=0D1117" />

</div>

---

## ⚡ About Me

```python
class SergioPoli:
    name        = "Sergio Poli"
    role        = "LLM Developer & AI Engineer"
    location    = "Indonesia 🇮🇩"

    focus       = [
        "LLM Integration & Prompt Engineering",
        "RAG Systems & Vector Databases",
        "AI Chatbots & Automation Workflows",
        "Fullstack Web Development"
    ]

    stack       = {
        "AI/ML"     : ["LangChain", "OpenAI API", "RAG", "Vector DBs"],
        "Backend"   : ["Node.js", "Express.js"],
        "Frontend"  : ["React.js"],
        "Database"  : ["PostgreSQL", "MongoDB", "Firebase"],
        "Messaging" : ["WhatsApp Gateway API"]
    }

    currently   = "Building AI-powered apps that actually solve real problems 🚀"
    fun_fact    = "I automate the boring stuff so humans can focus on what matters 🤖"
```

---

## 🏆 Flagship Project

<div align="center">

### 🤖 AI-Powered Citizen Complaint System

</div>

> 🌟 **Signature Project** — Bridging citizens and government through conversational AI

A smart digital assistant enabling citizens to submit complaints via **WhatsApp**, with AI-driven understanding, automated routing, and instant responses — helping public institutions respond to community issues **faster and smarter**.

```
📲 Citizen sends complaint via WhatsApp
        ↓
🧠 LLM processes & understands the issue (LangChain)
        ↓
🔀 Auto-categorized & routed to the right department
        ↓
✅ Automated acknowledgment + follow-up to citizen
```

<table>
  <tr>
    <td>🤖 <b>LLM Integration</b></td>
    <td>Intelligent understanding & automated responses via OpenAI</td>
  </tr>
  <tr>
    <td>📲 <b>WhatsApp Gateway</b></td>
    <td>Citizens report issues directly through familiar chat interface</td>
  </tr>
  <tr>
    <td>🔗 <b>LangChain Framework</b></td>
    <td>Manages LLM workflows, memory, and conversation chains</td>
  </tr>
  <tr>
    <td>☁️ <b>Scalable Backend</b></td>
    <td>Built on Express.js with modern architecture</td>
  </tr>
</table>

---

## 🏅 Other Notable Projects

<table>
<tr>
<td width="50%">

### 📄 AI Document Assistant (RAG)
Upload any document and have a full AI-powered Q&A session with its content.

**Highlights:**
- 📂 Document upload & processing pipeline
- 🔍 Retrieval-Augmented Generation (RAG)
- 💬 Interactive Q&A interface
- 🧠 Semantic search over document content

</td>
<td width="50%">

### 🫁 TBC WhatsApp Chatbot
AI chatbot providing Tuberculosis health information through WhatsApp.

**Highlights:**
- 🤖 LLM-powered health responses
- 💬 WhatsApp-native interface
- 📋 Auto-answers common TBC questions
- 🏥 Accessible health info via messaging

</td>
</tr>
<tr>
<td width="50%">

### 🛒 POS / Point-of-Sale System
Web-based sales and inventory management platform.

**Highlights:**
- 💰 Transaction management
- 📦 Product & inventory tracking
- 📊 Sales reporting & analytics

</td>
<td width="50%">

### 🌾 Agriculture Management System
Digital platform for managing agricultural operations.

**Highlights:**
- 🌱 Crop tracking & harvest monitoring
- 🧪 Fertilizer usage records
- 📋 Agricultural inventory management

</td>
</tr>
<tr>
<td width="50%">

### 🧑‍💼 Employee Attendance System
Smart attendance system with selfie verification check-in.

**Highlights:**
- 🤳 Selfie-based attendance verification
- 📊 Admin monitoring dashboard
- ⏱️ Real-time attendance records

</td>
<td width="50%">

### 🍽️ À L'AISE by the Lake
Website for a restaurant and tourism experience platform.

**Highlights:**
- 🖼️ Restaurant gallery & info
- ⛵ Experience booking (sailing, pottery, etc.)
- 💳 Payment gateway integration

</td>
</tr>
</table>

---

## 🛠️ Tech Stack

<div align="center">

### 🤖 AI & Machine Learning
![LangChain](https://img.shields.io/badge/LangChain-000000?style=for-the-badge&logo=chainlink&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI_API-412991?style=for-the-badge&logo=openai&logoColor=white)
![RAG](https://img.shields.io/badge/RAG_Systems-6A1B9A?style=for-the-badge&logo=databricks&logoColor=white)
![Vector DB](https://img.shields.io/badge/Vector_Databases-FF6B6B?style=for-the-badge&logo=pinecone&logoColor=white)

### 💻 Frontend & Backend
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white)

### 🗄️ Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)

### 🔌 Integrations & Tools
![WhatsApp](https://img.shields.io/badge/WhatsApp_API-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)

</div>

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats-eight-theta.vercel.app/api?username=sergiopoli77&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=61DAFB&icon_color=61DAFB&text_color=8b949e&border_radius=10" height="180px"/>
&nbsp;&nbsp;
<img src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=sergiopoli77&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=61DAFB&text_color=8b949e&border_radius=10" height="180px"/>

<br/><br/>

<img src="https://streak-stats.demolab.com/?user=sergiopoli77&theme=tokyonight&hide_border=true&background=0D1117&stroke=61DAFB&ring=61DAFB&fire=FF6B6B&currStreakLabel=61DAFB&sideLabels=8b949e&dates=8b949e&border_radius=10" height="180px"/>

</div>

---

## 🐍 Contribution Graph

<div align="center">
  <img src="https://raw.githubusercontent.com/sergiopoli77/sergiopoli77/output/github-contribution-grid-snake-dark.svg" alt="Snake animation" />
</div>

> ⚙️ **To activate the snake animation**, add a GitHub Actions workflow to your repo. See setup below ↓

<details>
<summary>🔧 <b>Setup Snake Animation (click to expand)</b></summary>

Create `.github/workflows/snake.yml` in your profile repo:

```yaml
name: Generate Snake

on:
  schedule:
    - cron: "0 */12 * * *"
  workflow_dispatch:

jobs:
  build:
    runs-on: ubuntu-latest
    steps:
      - uses: Platane/snk/svg-only@v3
        with:
          github_user_name: ${{ github.repository_owner }}
          outputs: |
            dist/github-contribution-grid-snake.svg
            dist/github-contribution-grid-snake-dark.svg?palette=github-dark
      - uses: crazy-max/ghaction-github-pages@v3.1.0
        with:
          target_branch: output
          build_dir: dist
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
```

</details>

---

## 🎯 What I Build

<div align="center">

| 🤖 AI Applications | 🧠 RAG Systems | 🔌 Integrations | 💻 Fullstack Platforms |
|:---:|:---:|:---:|:---:|
| LLM-powered apps that understand natural language | Intelligent assistants that retrieve & reason over documents | Connecting LLMs with APIs, messaging systems & automation | End-to-end web systems for AI-powered workflows |

</div>

---

## 💬 Let's Connect

<div align="center">

<a href="https://discord.com/users/581871647066423297">
  <img src="https://img.shields.io/badge/Discord-Chat%20with%20me-7289DA?style=for-the-badge&logo=discord&logoColor=white&labelColor=0D1117"/>
</a>
&nbsp;
<a href="https://instagram.com/sergiooyp">
  <img src="https://img.shields.io/badge/Instagram-Follow%20me-E4405F?style=for-the-badge&logo=instagram&logoColor=white&labelColor=0D1117"/>
</a>

<br/><br/>

> *"Technology is best when it brings people together."* — Matt Mullenweg

<br/>

<!-- FOOTER WAVE -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:16213e,50:1a1a2e,100:0D1117&height=120&section=footer&animation=fadeIn" />

</div>
